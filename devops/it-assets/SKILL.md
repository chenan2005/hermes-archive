---
name: it-assets
title: IT 资产清单 & SSH 免密登录配置
description: 家庭内网设备查询/操作/维护一体化入口。资产清单 + 子 skill 索引（android-termux-dev/winrm-ssh/windows-remote-control/wol-wake/archive-system/network-pitfalls）+ 代理优先级 + 维护原则。查设备/SSH/操作/维护从这里出发，按设备类型引用子 skill。
---
...
| **代理** | **Xray v26.6.1**（v2rayN v7.22.7 GUI 保留但不用），VLESS+Reality → 43.108.41.245:40002。SOCKS5 `0.0.0.0:10808`，计划任务 `Xray-SOCKS5`（SYSTEM 开机自启+崩溃重启）。静态路由 `route -p add 43.108.41.245 → 192.168.1.1 metric 50` 强制 VLESS 走 WiFi/移动宽带。详见 `windows-proxy-client` skill §8。曾用 sing-box（8897/8890），2026-07-04 已替换。 |
