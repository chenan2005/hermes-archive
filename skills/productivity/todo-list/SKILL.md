...
- s8 [2026-07-06] Memex 个人知识助手方案设计 | session:20260706_201822_0dfb10 | 暂无持久文件（方案讨论中） | 定名 Memex。定位：不新建存储系统，Hermes+Hindsight 为外脑界面。存储方案：Hindsight摘要 + LLM整理Markdown + 原始链接三层。处理流水线：收藏→系统即时处理（视觉LLM消化图文/ASR转视频→LLM整理出摘要+标签+结构化正文+关键观点+相关阅读+阅读建议+操作建议）→Markdown文件+Hindsight retain。用户状态机：unread→reading→digested（非inbox→processing→done；系统处理是即时的，状态反映用户消化程度）。YAML frontmatter含source/url/title/tags/status/notes/summary等字段，status和notes用户随时可变。待定：操作建议写到什么深度、视频时间戳是否保留。
