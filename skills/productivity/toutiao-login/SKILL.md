...
| 登录按钮页面坐标 | `(1131, 345)` | CDP dispatchMouseEvent 用页面坐标，如果改版需重新查找 |
| QR 定位 | CDP `getBoundingClientRect()` | 动态获取，不依赖固定像素坐标，页面改版后自动适配 |
...
脚本自动完成：
1. CDP 连接 → ESC 关闭弹窗 → 点击登录按钮 `(1131, 345)`
2. 等待 3 秒让弹窗渲染
3. **CDP 获取 QR 精确位置** — `getBoundingClientRect()` 拿 wrapper 坐标 + `outerH - innerH` 算 titlebar 高度
4. `import -window` 截取 Edge 窗口
5. Pillow 动态裁剪 QR 区域（坐标来自步骤 3，不依赖硬编码）
6. 飞书 API 上传图片并发送
...
- **import 坐标偏移** — `import -window` 捕获包含 titlebar 的窗口像素，titlebar 高度通过 `outerH - innerH` 动态计算
- **QR 改版** — 如果 `.web-login-scan-code__content__qrcode-wrapper` 选择器失效，需通过 DevTools 重新确认选择器
- **登录按钮改版** — 如果 `(1131, 345)` 坐标偏移，需通过 DevTools 重新获取
