...
ARCHIVE_SCHEMA = {
    ...
    "description": (
        ...
        "分析会话内容，按话题分组后使用此工具。\n\n"
        "操作:\n"
        "  archive   — 写入新话题组。新建传 title/description/summary；\n"
        "               merge_into=<gid> 合并到已有话题（会用新值覆盖原 title/description/summary）。\n"
        "               当前会话传 source_message_indices（0-based 数组索引，system prompt=0），\n"
        "               历史会话传 message_ids（session_search 返回的 state.db ID）\n"
        "  ls        — 列出所有已归档话题（gid + 描述）\n"
        ...
        "最佳实践:\n"
        "  \u2022 归档前先 archive(show, gid=[...]) 查看已有话题摘要，\n"
        "    然后一次性 archive(groups=[...]) 写入。\n"
