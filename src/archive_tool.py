...
ARCHIVE_SCHEMA = {
    ...
    "description": (
        ...
        "ACTIONS:\n"
        "  archive   \u2014 Write new topic groups (use merge_into=<gid> to merge into\n"
        "               an existing topic; omit merge_into to create new).\n"
        "               Pass source_message_indices (0-based array positions where\n"
        "               system prompt = index 0) for current-session archiving, or\n"
        "               message_ids (state.db IDs from a prior session_search)\n"
        "               for past-session archiving.\n"
        "  ls        \u2014 List all archived topics with gid + description.\n"
        ...
        "               (and optional profile). Returns all messages at once \u2014 use\n"
        "               this instead of multiple session_search scrolls when you\n"
        "               need the full transcript before archiving it.\n\n"
        "BEST PRACTICE:\n"
        "  \u2022 When archiving, first call archive(show, gid=[...]) to read existing\n"
        "    topic summaries, then write everything in one archive(groups=[...]) call.\n"
        "  \u2022 After 30+ rounds of Q&A on a single topic with conclusions reached,\n"
        "    proactively archive it. Also respond to explicit commands like 'archive'.\n"
        "  \u2022 When the conversation has covered 3+ distinct technical topics\n"
        "    (>=5 user messages each, some reaching a conclusion), proactively\n"
        "    archive them.\n"
        "  \u2022 For merges, produce a consolidated academic-style summary that\n"
        "    covers both old and new content.\n"
        "  \u2022 Set project=\"hermes-agent\" for topics about configuring/extending/\n"
        "    troubleshooting Hermes Agent itself. Omit project (or infer from CWD\n"
        "    git repo) for other topics."
    ),
    "parameters": {
        "type": "object",
        "properties": {
